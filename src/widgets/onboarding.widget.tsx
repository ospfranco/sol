...
export const OnboardingWidget: FC<Props> = observer(({ style }) => {
	...
	}, [store.ui.onboardingStep, store.ui.focusWidget]);
				...
				<Fade visible={visible} className="items-center flex-1" duration={250}>
					<View className="flex-1 justify-center relative">
						...
						{SHORTCUTS.map((item, index) => {
							...
							const selected = store.ui.selectedIndex === index;

							return (
								<View key={`${index}-button`} className="items-center">
									<View
										className={clsx("flex-row items-center px-3 py-2 rounded", {
